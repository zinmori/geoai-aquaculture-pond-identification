...
**Final result: 9th place. Private score 0.947, public score 0.942367202.**
...
| | value |
|---|---|
| **Private score (721 rows)** | **0.947 — 9th place** |
...
(`python -u run_v9.py > log.txt 2>&1`) gives a complete audit trail of a run; the
...
* **We measured it with CodeCarbon** (v3.3.0, `codecarbon monitor --no-api --
  python run_v9.py`, so the pipeline code is untouched). The raw report is in
...
python -u run_v9.py > log.txt 2>&1       # 3 h 15 min on a laptop CPU
```
